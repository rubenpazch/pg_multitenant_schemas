...
RSpec.describe "Cookie Persistence" do
  ...
  describe "HttpOnly flag" do
    ...
    it "is set when storing schema cookie" do
      # When storing the cookie, http_only should be false to allow JS access
      cookie_options = { http_only: false }

      expect(cookie_options[:http_only]).to be(false)
    end
  end
