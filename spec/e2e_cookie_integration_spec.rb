...
RSpec.describe "End-to-End Multitenant Cookie-Based Schema Switching" do
  ...
  before do
    ...
    allow(PgMultitenantSchemas::SchemaSwitcher).to receive(:schema_exists?).and_return(true)
  end
  ...
  describe "cross-app communication via cookie" do
    it "allows dashboard to communicate schema selection to main app" do
      ...
      expect(PgMultitenantSchemas::SchemaSwitcher).to receive(:schema_exists?).with(schema_from_cookie).and_return(true)
